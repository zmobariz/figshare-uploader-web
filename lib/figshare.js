...
async function rawFetch(url, options = {}, { retries = 4, onRetry } = {}) {
  // SSRF barrier (inline, so static analysis recognises it): only HTTPS requests
  // to Figshare hosts are ever issued. Private/loopback/other hosts are not
  // Figshare hosts, so they are rejected before fetch() is ever reached.
  let parsed;
  try { parsed = new URL(String(url)); } catch (e) { const err = new Error('Invalid URL'); err.status = 400; throw err; }
  const host = parsed.hostname.toLowerCase();
  const allowed = parsed.protocol === 'https:' && (
    host === 'figshare.com' || host.endsWith('.figshare.com') ||
    host === 'figsh.com' || host.endsWith('.figsh.com'));
  if (!allowed) { const err = new Error('Blocked non-Figshare request: ' + host); err.status = 400; throw err; }
  let attempt = 0;
  for (;;) {
    let res;
    try {
